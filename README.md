...
`adc-compliance-checker` is a python based tool for those wanting to submit data to the Artic Data Centre. The python module can be used to check if files fullfill the metadata requirements listed [here](https://adc.met.no/node/4).
...
Install the `adc-compliance-checker` module via `pip`:

```sh
pip install adc-compliance-checker
```
